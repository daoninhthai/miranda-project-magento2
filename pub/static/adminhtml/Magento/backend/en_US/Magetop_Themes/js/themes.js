	...
	(function($){
		jQuery(document).ready(function($){
			...
			function getDirMcolorpicker () {
					...
					var dir = detected!==false ? detected : 'themes/';
				return dir;
			}
			...
			$(".color, .mb-color, .mc-color").attr("data-hex", true).width("116px").mColorPicker();
		  	jQuery(document).on({ 
		  		mouseover: function() {jQuery.fn.mColorPicker.events(); $('#mColorPickerWrapper').hide()} 
		  	});
		  	$('body').click(function(event) {$('#mColorPicker').hide();});

			var color = '#row_mbdesign_base_color';
			var background = '#row_mbdesign_base_background';
			var border = '#row_mbdesign_base_border';
		    ...
		    $(selector).each(function(index, el) {
		    	...
		    	$(this).find(".grid, .design_theme_ua_regexp").width("1024px");
		    });
