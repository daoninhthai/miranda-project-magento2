 ...
 **/
namespace Magetop\Marketplace\Controller\Product\Initialization;
...
class Helper {
        ...
        if ($productOptions && !$product->getOptionsReadonly()) {
            ...
            foreach ($options as $customOptionData) {
                if (empty($customOptionData['is_delete'])) {
					...
					$customOptionData['record_id'] = $customOptionData['option_id'];
					if (empty($customOptionData['option_id']) || $customOptionData['option_id'] == 0) {
    ...
    public function mergeProductOptions($productOptions, $overwriteOptions)
    {
			...
			foreach ($productOptions as $optionIndex => $option) {	
				if (isset($option['values'])) {
					...
					foreach ($option['values'] as $valueIndex => $value) {
						if ($option['values'][$valueIndex]['option_type_id'] == -1) {
							$option['values'][$valueIndex]['record_id'] = $record_id;
							unset($option['values'][$valueIndex]['option_type_id']);
							$record_id++;						
						}
					}				
				}
        ...
        foreach ($productOptions as $optionIndex => $option) {
			...
			$option_type_id = $option['values'][$valueIndex]['option_type_id'];
                ...
                foreach ($option['values'] as $valueIndex => $value) {
                    if (isset($value['option_type_id'])) {
						if ($value['option_type_id'] != -1) {
							...
							$option['values'][$valueIndex]['record_id'] = $option['values'][$valueIndex]['option_type_id'];
