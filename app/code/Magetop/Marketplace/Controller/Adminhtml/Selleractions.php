            ...
            foreach ($linkTypes as $type) {
                if (isset($links[$type])) {
                    ...
                    foreach($links[$type] as $key => $value){
                        $validateProduct = $this->_objectManager->create('Magetop\Marketplace\Model\Products')->getCollection()->addFieldToFilter('product_id',$key)->getFirstItem();
                        if($validateProduct['id'] && ($validateProduct['user_id'] != $model->getUserId())){
                    		...
                    		$sellerModel = $objectManager->create('Magetop\Marketplace\Model\Sellers')->getCollection()->addFieldToFilter('user_id',$validateProduct['user_id'])->getFirstItem();
