...
class GetSellerDashboard extends \Magetop\Api\Controller\AbstractController
{
    ...
    public function execute(){
        ...
        try{
            ...
            $partnerAmount = $this->getPartnerAmount();
            $lastOrder = $this->getLastOrder();
            $reviews = $this->getLatestReview();
        	$month_sale = $this->getMonthlysale();
            
            $lastOrdersData = array();
            if(count($lastOrder)){
                foreach($lastOrder as $order){
                    $lastOrdersData[] = array(
                        ...
                        'grand_total' => $this->getMkPriceHelper()->currency( $order->getTotalamount() ,true,false),
                        'status' => $order->getStatus(),
                        'actions' => ''
                    );
                }
            }
            $bestsale = $this->getBestSellerProducts();
            $bestSellersData = array();
            if(count($bestsale)){
                foreach($bestsale as $product){
                    $bestSellersData[] = array(
                        'product_id' => $product->getId(),
                        'product_name' => $product->getName(),
                        'price' => $this->getMkPriceHelper()->currency( $product->getFinalPrice() ,true,false),
                        'quantity' => (int)$product->getQtyOrdered()
                    );
                }
            }
            $lastReviewsData = array();
            if(count($reviews)){
                foreach($reviews as $review){
                    $lastReviewsData[] = array(
                        'product_id' => $review->getId(),
                        'time' => $review->getCreatedate(),
                        'reviewer' => $review->getFirstname().' '.$review->getLastname(),
                        'rating_price' => $review->getPrice(),
                        'rating_value' => $review->getValue(),
                        'rating_quality' => $review->getQuality(),
                        'comment' => $review->getReview()
                    );
                }
            }
            $data = array(
                'credit_amount' => $this->getMkPriceHelper()->currency($partnerAmount['amountremain'],true,false),
                'total_sales' => $this->getMkPriceHelper()->currency( $this->getTotalAmount(),true,false),
                'total_orders' => $this->getTotalOrders(),
                'total_products' => $this->getTotalProducts()->count(),
                'today_sales' => $this->getTodaySales(),
                'today_orders' => $this->getMkPriceHelper()->currency($this->getTodayAmount(),true,false),
                'your_income' => array(
                    'week' => $this->getMkPriceHelper()->currency($this->getWeekAmount(),true,false),
                    'month' => $this->getMkPriceHelper()->currency($this->getMonthAmount(),true,false),
                    'year' => $this->getMkPriceHelper()->currency($this->getYearAmount(),true,false)
                ),
                'remaining_amount' => $this->getMkPriceHelper()->currency($partnerAmount['amountremain'],true,false),
                'total_payout' => $this->getMkPriceHelper()->currency($partnerAmount['amountreceived'],true,false),
                'orders' => array(
                    'total' => $this->getOrderStatus( 'all' ),
                    'complete' => $this->getOrderStatus( 'complete' ),
                    'pending' => $this->getOrderStatus( 'pending' ),
                    'processing' => $this->getOrderStatus( 'processing' ),
                    'canceled' => $this->getOrderStatus( 'canceled' ),
                    'holded' => $this->getOrderStatus( 'holded' )
                ),
                'last_orders' => $lastOrdersData,
                'best_sellers' => $bestSellersData,
                'last_reviews' => $lastReviewsData
            );
        }catch(\Exception $e){
            $status = false;
