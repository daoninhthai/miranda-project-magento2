...
abstract class Actions extends \Magento\Backend\App\Action
{
    ...
    protected function _massStatusAction()
    {
        ...
        try {
            ...
            foreach($ids as $id) {
                $attribute_name = $id;
                $data = $this->_objectManager->create($this->_modelClass)->getCollection()->addFieldToFilter('attribute_name',$attribute_name);
                if(count($data)){
                    foreach($data as $row){
                    ...
                    $productAttributeModel = $this->_objectManager->create($this->_modelClass);
                    $productAttributeModel->setData('attribute_name', $attribute_name);
                    $productAttributeModel->setData('status', $status);
